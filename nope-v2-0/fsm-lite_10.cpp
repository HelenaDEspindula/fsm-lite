...
void wt_init(wt_t &wt, bitv_t &separator, cst_t &cst, input_reader *ir, configuration &config)
{
  ...
  {
    cerr << "[DEBUG]Labeling failed, j = " << j << ", k = " << k << endl;
    exit(1);
  }
  ...
  if (j != n) // Assert
  {
    cerr << "[DEBUG]Label sum failed, j = " << j << ", n = " << n << endl;
    exit(1);
  }
...
int main(int argc, char ** argv)
{
  ...
  if (config.verbose)
    cerr << "[DEBUG]Reading input files..." << endl;
  input_reader *ir = input_reader::build(config);
  if (config.verbose)
    cerr << "[DEBUG]Read " << ir->size() << " input files and " << ir->total_seqs() << " sequences of total length " << ir->total_size() << " (includes rev.compl. sequences)" << endl;
  
  /**
   * Initialize the data structures
   */
  if (config.verbose)
    cerr << "[DEBUG]Constructing the data structures..." << endl;
  cst_t cst;    
  construct(cst, config.tmpfile + ".tmp", 1);
  if (!cst.csa.size())
  {
    cerr << "[DEBUG]error: unable to construct the data structure; out of memory?" << endl; 
    abort();
  }
  ...
  if (config.verbose)
    cerr << "[DEBUG]Construction complete, the main index requires " << size_in_mega_bytes(cst) << " MiB plus " << size_in_mega_bytes(label_wt) << " MiB for labels." << endl;
  ...
  while (!buffer.empty())
  {
    ...
    /*if (config.debug)
     ...
     cerr << "[DEBUG]at node = " << depth << "-[" << sp << "," << ep << "], wl = " << (wn != root);
     if (wn!=root)
     cerr << "[DEBUG][" << cst.rb(wn)-cst.lb(wn) << " vs " << ep-sp << "]";
     cerr << "[DEBUG], seq = " << s << endl;
     }*/
    if (wn == root && config.debug)
    {
      cerr << "[DEBUG]warning: no Weiner-link at " << depth << "-[" << sp << "," << ep << "]" << endl;
      continue;
    }
