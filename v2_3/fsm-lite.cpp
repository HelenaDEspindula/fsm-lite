...
void wt_init(wt_t &wt, bitv_t &separator, cst_t &cst, input_reader *ir, configuration &config)
{
  ...
  /* Modified in Version 2.4 */
  if (j != n)
  {
    cerr << "Label sum failed, j = " << j << ", n = " << n << endl;
    if (config.verbose)
      cerr << "[ERROR] Error in wt_init: total number of labels does not match the size of the processed text. Possible corruption or lack of memory." << endl;
    exit(1);
  }
  /* --- */
}
