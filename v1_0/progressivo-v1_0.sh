...
for N in "${GENOMAS[@]}"; do
  ...
  for J in "${SMAIUSCULO[@]}"; do
    ...
    # OUTPUT_LOG="${LOG_DIR}/output/fsm_output_log_v${VERSION}_${N}genomas_${J}_max_TXT--${TIMESTAMP}.txt"
    # TMP_FILES="${TMP_DIR}/fsm_tmp_files_v${VERSION}_${N}genomas_${J}_max_TXT--${TIMESTAMP}"
    # OUTPUT_RES="${RES_DIR}/fsm_results_v${VERSION}_${N}genomas_${J}_max_TXT--${TIMESTAMP}--2.txt"
    # 
    # echo "Linha com problema"
    # 
    # # Executar fsm-lite em background
    # (/usr/bin/time ./fsm-lite -s $SMINUSCULO -S $SMAIUSCULO -m $MMINUSCULO -l "$INPUT_FILE" -t "$TMP_FILES" > $OUTPUT_RES) & FSM_PID=$!
    # 
    #  ## 2> "$OUTPUT_LOG" ) 
