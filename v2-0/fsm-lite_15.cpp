...
void wt_init(wt_t &wt, bitv_t &separator, cst_t &cst, input_reader *ir, configuration &config)
{
  ...
  if (config.debug)
    cerr << "[CERR-DEBUG] bwt end marker pos = " << j << endl;
  ...
  {
    cerr << "[CERR-DEBUG] Labeling failed, j = " << j << ", k = " << k << endl;
    exit(1);
  }
  ...
  if (config.debug)
    cerr << "[CERR-DEBUG] wt size = " << wt.size() << ", n = " << n << endl;
  j = 0;
  for (uint64_t i = 0; i < ir->size(); ++i)
    j += wt.rank(n, i);
  if (j != n) // Assert
  {
    cerr << "[CERR-DEBUG] Label sum failed, j = " << j << ", n = " << n << endl;
    exit(1);
  }
...
int main(int argc, char ** argv)
{
  ...
  if (config.verbose)
    cerr << "[CERR-DEBUG] Reading input files..." << endl;
  input_reader *ir = input_reader::build(config);
  if (config.verbose)
    cerr << "[CERR-DEBUG] Read " << ir->size() << " input files and " << ir->total_seqs() << " sequences of total length " << ir->total_size() << " (includes rev.compl. sequences)" << endl;
  
  /**
   * Initialize the data structures
   */
  if (config.verbose)
    cerr << "[CERR-DEBUG] Constructing the data structures..." << endl;
  cst_t cst;    
  construct(cst, config.tmpfile + ".tmp", 1);
  if (!cst.csa.size())
  {
    cerr << "[CERR-DEBUG] error: unable to construct the data structure; out of memory?" << endl; 
    abort();
  }
  ...
  if (config.verbose)
    cerr << "[CERR-DEBUG] Construction complete, the main index requires " << size_in_mega_bytes(cst) << " MiB plus " << size_in_mega_bytes(label_wt) << " MiB for labels." << endl;
  ...
  while (!buffer.empty())
  {
    ...
    if (config.debug)
     ...
     cerr << "[CERR-DEBUG] at node = " << depth << "-[" << sp << "," << ep << "], wl = " << (wn != root);
     if (wn!=root)
     cerr << "[CERR-DEBUG] [" << cst.rb(wn)-cst.lb(wn) << " vs " << ep-sp << "]";
     cerr << "[CERR-DEBUG] , seq = " << s << endl;
     }
    // -----
    if (wn == root && config.debug)
    {
      cerr << "[CERR-DEBUG] warning: no Weiner-link at " << depth << "-[" << sp << "," << ep << "]" << endl;
      continue;
    }
