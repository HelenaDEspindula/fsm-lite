...
int main(int argc, char ** argv)
{
  ...
  if (config.verbose)
    cerr << "All done." << endl;    
  delete ir; ir = 0;
            ++total_support_positive;
        }
    }

    // Bloco de diagnóstico de suporte
    size_t total_support_positive = 0;
    for (size_t i = 0; i < support.size(); ++i) {
        if (support[i] >= config.m) {
            ++total_support_positive;
        }
    }
    cerr << "[DEBUG] Nós com suporte >= " << config.m << ": " << total_support_positive << endl;
